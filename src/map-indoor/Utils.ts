...
export function filtersByDatas(datas: any) {
  const uniqueCatsByLayer = {};

  datas.features.forEach((item) => {
    const { cat, layer } = item.properties;

    if (cat && cat.length > 0) {
      if (!uniqueCatsByLayer[layer]) {
        uniqueCatsByLayer[layer] = new Set(); 
      }

      cat.forEach((catValue) => {
        uniqueCatsByLayer[layer].add(catValue);
      });
    }
  });

  const result = Object.keys(uniqueCatsByLayer).map((layer) => ({
    layer,
